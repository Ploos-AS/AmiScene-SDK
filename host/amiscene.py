...
from amiscene.image import build_png,emit_palette_asm
...
def main():
    ...
    im=sub.add_parser("image");im.add_argument("input");im.add_argument("--planes",type=int,required=True);im.add_argument("--layout",choices=("interleaved","separate"),default="interleaved");im.add_argument("--output",required=True);im.add_argument("--asm");im.add_argument("--label",default="bitmap")
    ...
    if a.command=="image":
        asset=build_png(a.input,a.planes,a.layout);Path(a.output).write_bytes(asset["data"])
        if a.asm:
            asm=emit_incbin(Path(a.output).name,a.label)+emit_palette_asm(asset["palette"],a.label+"_palette")
            asm += a.label+"_width equ %d\n%s_height equ %d\n%s_planes equ %d\n" % (asset["width"],a.label,asset["height"],a.label,asset["planes"])
            Path(a.asm).write_text(asm)
        return 0
