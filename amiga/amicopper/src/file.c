...
int AmiCopperDocumentLoad(struct AmiCopperDocument *doc,const char *path)
{
 ...
 fclose(f); if(!AmiCopperDocumentValidate(&tmp,0))return 0; *doc=tmp; doc->path=path; doc->dirty=0; return 1;
}
