...
static void draw_shell(struct Window *w, struct AmiCopperUI *ui)
{
 ...
 sprintf(line,"M0 %s/%s | Tab edit F5 validate Esc quit",ui->document->dirty?"DIRTY":"CLEAN",ui->document->valid?"VALID":"UNVALIDATED");Move(rp,12,190);Text(rp,(STRPTR)line,(LONG)strlen(line));
}
...
void AmiCopperUIRun(struct AmiCopperUI *ui){struct Window*w=(struct Window*)ui->window;refresh_selected(ui);draw_shell(w,ui);while(ui->running){struct IntuiMessage*msg;Wait(1UL<<w->UserPort->mp_SigBit);while((msg=(struct IntuiMessage*)GetMsg(w->UserPort))){ULONG cls=msg->Class;UWORD code=msg->Code,qual=msg->Qualifier;ReplyMsg((struct Message*)msg);if(cls==IDCMP_REFRESHWINDOW){BeginRefresh(w);draw_shell(w,ui);EndRefresh(w,TRUE);continue;}if(cls==IDCMP_CLOSEWINDOW){ui->running=0;continue;}if(cls!=IDCMP_RAWKEY)continue;
 ...
 if(code==0x4c&&ui->document->selected)ui->document->selected--;else if(code==0x4d&&ui->document->selected+1<ui->document->instruction_count)ui->document->selected++;else if(code==0x37)AmiCopperDocumentInsert(ui->document,0x0180,0);else if(code==0x11)AmiCopperDocumentInsert(ui->document,0x2c01,0xfffe);else if(code==0x21)AmiCopperDocumentInsert(ui->document,0x2c01,0xffff);else if(code==0x46)AmiCopperDocumentDelete(ui->document);else if(code==0x50){unsigned long bad=0;if(!AmiCopperDocumentValidate(ui->document,&bad)&&bad<ui->document->instruction_count)ui->document->selected=bad;}else if(code==0x42){ui->editing=1;AmiCopperFormatSource(&ui->selected,ui->edit,sizeof(ui->edit));ui->edit_len=(unsigned int)strlen(ui->edit);}else if(code==0x45)ui->running=0;refresh_selected(ui);draw_shell(w,ui);
 }}}
